<?php
class MMovie_Choreographer extends CI_Model
{
    ...
    function bind_dropdown_Movie_Choreographer()
    {
		$data = array();
     	$this->db->select('Choreographer_id,Choreographer_name');
     	  $this->db->order_by("Choreographer_name", "asc"); 
    ...
    function fill_Movie_Choreographer_table($alpha,$keyword)
    {
		...
		$Q = $this->db->query('SELECT * FROM Movie right join Movie_Choreographer on (movie_Choreographer.Movie_Id = Movie.Movie_Id) WHERE (movie.Movie_name LIKE "'.$alpha.'%" AND movie.Movie_name LIKE "'.$keyword.'%") ');
		return $Q->num_rows();
   	}
   

    function fill_Movie_Choreographer_table_condition($start_row,$limit,$alpha,$keyword)
    {
		if($alpha == "ALL")
			$alpha = "";
		if($alpha == "0-9")
		...
		$query_str = 'SELECT * FROM Movie right join Movie_Choreographer on (movie_Choreographer.Movie_Id = Movie.Movie_Id) left join Choreographer on (Choreographer.Choreographer_id = movie_Choreographer.Choreographer_id) WHERE (movie.Movie_name LIKE "'.$alpha.'%" AND movie.Movie_name LIKE "'.$keyword.'%")  LIMIT '.$start_row.','.$limit;
	    $Q = $this->db->query($query_str);
	    return $Q->result();
   	}
   
    
    function fill_Movie_Choreographer_details($Movie_Choreographer_id)
    {
		$this->db->where('Movie_Choreographer_id',$Movie_Choreographer_id);
			
   		$Q = $this->db->get('Movie_Choreographer');
    ...
    function save_new_Movie_Choreographer()
    {
			...
			//	'Gender'=>($this->input->post('chkIsMale')==='on')?1:0,
				'Choreographer_id'=>$this->input->post('ddlChoreographer'),
				

			);
			$this->db->insert('Movie_Choreographer',$d);
    ...
    function update_Movie_Choreographer()
    {
   		$Movie_Choreographer_id = $this->input->get('Movie_Choreographer_id');
			...
			$this->db->where('Movie_Choreographer_id',$Movie_Choreographer_id);
			$this->db->update('Movie_Choreographer',$d);
    ...
    function check_existance_of_Movie_Choreographer($Choreographer_name,$Choreographer_id)
    {
