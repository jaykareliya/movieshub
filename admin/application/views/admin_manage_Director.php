...
{
    var answer = confirm("Are you sure you want to delete Director ?")
    if (answer){
...
$("document").ready(function(){
	$("#ddlRecordSelection").change(function(){
		location.replace("manage_Director?records="+$(this).val());
	});
...
<div style="margin:0 10px 15px 10px">
  <table id="table-search" cellpadding="0" cellspacing="0" border="0" width="100%">
    <tr>
      ...
      <td class="search"><?php
$alpha = "A";
echo '<a href="'.base_url().'index.php/Director/manage_Director?alpha=0-9" >#</a>&nbsp;';
echo '|&nbsp;';
for($i=0;$i<26;$i++)
{
	echo '<a href="'.base_url().'index.php/Director/manage_Director?alpha='.$alpha.'" >'.$alpha++.'</a>&nbsp;';
	echo '|&nbsp;';
}
echo '<a href="'.base_url().'index.php/Director/manage_Director?alpha=ALL" >ALL</a>';
?></td>
      <td align="right" ><a href="new_Director" class="btn-bg">Add Director</a></td>
    </tr>
  </table>
</div>
<div id="table-content">
<?php
$result = count($fill_director_table);
if($result == 0) 
{ 
?>
...
?>
  <table id="product-table" class="tablesorter" cellspacing="0" cellpadding="5" border="0" width="100%">
    <thead><tr bgcolor="#eaeaea">
      <?php  
		...
		{ 
			...
			<th class="table-header-repeat line-left"><a>Director Name</a></th>
    ...
    <tbody>
      <?php 
	  		$alterrow=0;
	  		$result = count($fill_director_table);if($result != 0) {
				foreach($fill_director_table as $row)
				{
				if($alterrow%2==0){echo "<tr class='alternate-row'>";}else{echo "<tr>";}
				echo "<td> <img src='".base_url()."../images/director/small/".$row->Director_Avatar."' />";
				echo "<td>".$row->Director_Name."</td>";			
			
				echo "<td align='center'><a href='new_Director?Director_id=".$row->Director_id."' class='icon-1 info-tooltip' title='Edit'></a>
					<a onclick='return deletechecked();' href='delete_Director?Director_id=".$row->Director_id."' class='icon-2 info-tooltip' title='Delete'></a></td>";
				echo "</tr>";
				$alterrow++;
				} 
			}
