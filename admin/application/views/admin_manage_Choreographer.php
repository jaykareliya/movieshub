...
$("document").ready(function(){
	$("#ddlRecordSelection").change(function(){
		location.replace("manage_Choreographer?records="+$(this).val());
	});
...
<div style="margin:0 10px 15px 10px">
  <table id="table-search" cellpadding="0" cellspacing="0" border="0" width="100%">
    <tr>
      ...
      <td class="search"><?php
$alpha = "A";
echo '<a href="'.base_url().'index.php/Choreographer/manage_Choreographer?alpha=0-9" >#</a>&nbsp;';
echo '|&nbsp;';
for($i=0;$i<26;$i++)
{
	echo '<a href="'.base_url().'index.php/Choreographer/manage_Choreographer?alpha='.$alpha.'" >'.$alpha++.'</a>&nbsp;';
	echo '|&nbsp;';
}
echo '<a href="'.base_url().'index.php/Choreographer/manage_Choreographer?alpha=ALL" >ALL</a>';
?></td>
      <td align="right" ><a href="new_Choreographer" class="btn-bg">Add Choreographer</a></td>
    </tr>
  </table>
</div>
<div id="table-content">
<?php
$result = count($fill_Choreographer_table);
if($result == 0) 
{ 
?>
...
?>
  <table id="product-table" class="tablesorter" cellspacing="0" cellpadding="5" border="0" width="100%">
    <thead><tr bgcolor="#eaeaea">
      <?php  
		...
		{ 
			...
			<th class="table-header-repeat line-left"><a>Choreographer Name</a></th>
    ...
    <tbody>
      <?php 
	  		$alterrow=0;
	  		$result = count($fill_Choreographer_table);if($result != 0) {
				foreach($fill_Choreographer_table as $row)
				{
				if($alterrow%2==0){echo "<tr class='alternate-row'>";}else{echo "<tr>";}
				echo "<td> <img src='".base_url()."../images/Choreographer/small/".$row->Choreographer_Avatar."' />";
				echo "<td>".$row->Choreographer_Name."</td>";			
			
				echo "<td align='center'><a href='new_Choreographer?Choreographer_id=".$row->Choreographer_Id."' class='icon-1 info-tooltip' title='Edit'></a>
					<a onclick='return deletechecked();' href='delete_Choreographer?Choreographer_id=".$row->Choreographer_Id."' class='icon-2 info-tooltip' title='Delete'></a></td>";
				echo "</tr>";
				$alterrow++;
				} 
			}
