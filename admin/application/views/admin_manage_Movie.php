...
{
    var answer = confirm("Are you sure you want to delete Movie ?")
    if (answer){
...
$("document").ready(function(){
	$("#ddlRecordSelection").change(function(){
		location.replace("manage_movie?records="+$(this).val());
	});
...
<div style="margin:0 10px 15px 10px">
  <table id="table-search" cellpadding="0" cellspacing="0" border="0" width="100%">
    <tr>
      ...
      <td class="search"><?php
$alpha = "A";
echo '<a href="'.base_url().'index.php/Movie/manage_Movie?alpha=0-9" >#</a>&nbsp;';
echo '|&nbsp;';
for($i=0;$i<26;$i++)
{
	echo '<a href="'.base_url().'index.php/Movie/manage_Movie?alpha='.$alpha.'" >'.$alpha++.'</a>&nbsp;';
	echo '|&nbsp;';
}
echo '<a href="'.base_url().'index.php/Movie/manage_Movie?alpha=ALL" >ALL</a>';
?></td>
      <td align="right" ><a href="new_Movie" class="btn-bg">Add Movie</a></td>
    </tr>
  </table>
</div>
<div id="table-content">
<?php
$result = count($fill_Movie_table);
if($result == 0) 
{ 
?>
...
?>
  <table id="product-table" class="tablesorter" cellspacing="0" cellpadding="5" border="0" width="100%">
    <thead><tr bgcolor="#eaeaea">
      <?php  
		...
		{ 
			...
			<th class="table-header-repeat line-left"><a>Movie Name</a></th>
    ...
    <tbody>
      <?php 
	  		$alterrow=0;
	  		$result = count($fill_Movie_table);if($result != 0) {
				foreach($fill_Movie_table as $row)
				{
				if($alterrow%2==0){echo "<tr class='alternate-row'>";}else{echo "<tr>";}
				
					echo "<td><img src='<?=base_url()?>/images/movie/small/".$row->Movie_Image."' /></td>";
				
				echo "<td>".$row->Movie_Name."</td>";			
			
				echo "<td align='center'><a href='new_Movie?Movie_id=".$row->Movie_Id."' class='icon-1 info-tooltip' title='Edit'></a>
					<a onclick='return deletechecked();' href='delete_Movie?Movie_id=".$row->Movie_Id."' class='icon-2 info-tooltip' title='Delete'></a></td>";
				echo "</tr>";
				$alterrow++;
				} 
			}
